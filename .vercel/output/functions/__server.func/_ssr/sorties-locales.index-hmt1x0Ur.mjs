...
import { $ as Earth, F as Download, T as MapPin, W as Building2, g as Search, o as Upload, s as Truck } from "../_libs/lucide-react.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/sorties-locales.index-hmt1x0Ur.js
...
function ImportSortiesDialog({ onClose, agencies, selectedYear, selectedMonth, onSuccess }) {
	...
	const parseCSV = (file) => {
		...
		reader.onload = (e) => {
			...
			const processedWholesalers = /* @__PURE__ */ new Set();
			for (let i = 0; i < rawHeaders.length; i++) {
				const match = rawHeaders[i].match(/^(.+?)\s*-\s*(ventes?|sales?|sortie|stocks?|commandes?|cmds?|orders?)$/i);
				if (match) {
					...
					if (!processedWholesalers.has(wholesalerName)) {
						...
						for (let j = 0; j < rawHeaders.length; j++) {
							...
							if (h.startsWith(wholesalerName + " -") || h.startsWith(wholesalerName + "-")) {
								...
								if (lower.includes("vente") || lower.includes("sales") || lower.includes("sortie")) salesIdx = j;
								else if (lower.includes("stock")) stockIdx = j;
								else if (lower.includes("cmd") || lower.includes("commande") || lower.includes("order")) ordersIdx = j;
							}
						}
			...
			if (isWideFormat) {
				...
				for (let i = 1; i < lines.length; i++) {
					...
					let productCip = "";
					if (cipIdx !== -1 && values[cipIdx]) productCip = values[cipIdx];
					else {
						const cipMatch = productName.match(/^(\d+)/);
						if (cipMatch) productCip = cipMatch[1];
						else productCip = productName.substring(0, 20).replace(/[^a-zA-Z0-9]/g, "");
					}
					for (const wc of wholesalerColumns) {
						const sales = wc.salesIdx !== -1 && values[wc.salesIdx] ? parseInt(values[wc.salesIdx].replace(/[^\d]/g, "")) || 0 : 0;
						const stock = wc.stockIdx !== -1 && values[wc.stockIdx] ? parseInt(values[wc.stockIdx].replace(/[^\d]/g, "")) || 0 : 0;
						const orders = wc.ordersIdx !== -1 && values[wc.ordersIdx] ? parseInt(values[wc.ordersIdx].replace(/[^\d]/g, "")) || 0 : 0;
						if (sales > 0 || stock > 0 || orders > 0) rows.push({
							productCip,
							productName,
